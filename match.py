...
def get_order(transaction, orders):
    ...
    if len(possible_orders) == 1:
        ...
        utils.log('ambiguous transaction has %s matches' % len(possible_orders), transaction, possible_orders)
...
def adjust_items(t, order, items):
    ...
    order_total = utils.float_from_amazon_price(order['Total Charged'])
    ...
    for i in items:
        ...
        i['Item Total'] = '$' + str(utils.float_from_amazon_price(i['Item Total']) * adjustment_ratio)
    new_item_total = sum(utils.float_from_amazon_price(i['Item Total']) for i in items)
