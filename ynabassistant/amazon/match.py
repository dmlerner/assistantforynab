...
def get_order(transaction, orders):
    ...
    if len(possible_orders) == 1:
        ...
        if ya.settings.fail_on_ambiguous_transaction:
            ya.utils.log_error('Skipping ambiguous transaction', transaction, possible_orders)
            return None
        else:
            ya.utils.log_debug('Skipping ambiguous transaction', transaction, possible_orders)
    ...
    ya.utils.log_info('Matched transaction with order', transaction, order)
    return order
