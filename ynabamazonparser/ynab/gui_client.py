...
def load_gui():
    ...
    d.get(url)
    if not yap.utils.gui.get('user-logged-in', require=False):
...
def enter_fields(fields, values):
    ...
    for i, (f, v) in enumerate(zip(fields, values)):
        ...
        f.send_keys(str(v))
        if i != len(fields) - 1:
            f.send_keys(yap.utils.gui.Keys.TAB)
...
def locate_transaction(t):
    ...
    search = yap.utils.gui.get('transaction-search-input')
    search.clear()
    search.send_keys('Memo: %s' % t.id)
    search.send_keys(yap.utils.gui.Keys.ENTER)


def add_subtransaction_rows(t):
    yap.utils.log_debug('add_subtransactions_rows', len(t.subtransactions))
    memo = yap.utils.gui.get_by_text('user-entered-text', t.id, count=1, partial=True)
    yap.utils.gui.click(memo, 2)
    removes = yap.utils.gui.get('ynab-grid-sub-remove', require=False, wait=1)
    while removes:
        yap.utils.gui.click(removes)
        removes = yap.utils.gui.get('ynab-grid-sub-remove', require=False, wait=.5)
    n = len(t.subtransactions)
    if n > 1:
        category_dropdown = yap.utils.gui.get_by_placeholder('dropdown-text-field', 'category')
        category_dropdown.send_keys(' ')
        split = yap.utils.gui.get('modal-account-categories-split-transaction')
        yap.utils.gui.click(split)
        'yap.utils.gui.clicking split means we already have two'
        for i in range(n - 2):
            yap.utils.gui.click(yap.utils.gui.get('ynab-grid-split-add-sub-transaction'))
...
def enter_transaction(t):
    ...
    account, date, payees, categories, memos = map(lambda p: yap.utils.gui.get_by_placeholder('accounts-text-field', p),
                                                   ('account', 'date', 'payee', 'category', 'memo'))
    date.send_keys(t._date)
    outflows, inflows = map(lambda p: yap.utils.gui.get_by_placeholder(
    ...
    if n == 1:
        ...
        approve = yap.utils.gui.get_by_text('button-primary', ['Approve', 'Save'])
        yap.utils.log_debug('approve/save?', approve.text)
        yap.utils.gui.click(approve)
    else:
        memos[0].clear()
        ...
        outflows[-1].send_keys(yap.utils.gui.Keys.ENTER)
