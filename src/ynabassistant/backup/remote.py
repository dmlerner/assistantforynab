...
def diff_with_backup(predicates, timestamp=settings.start_time, order='first'):
    ...
    Assistant.download_ynab(transactions=True)  # TODO: can I get away with this?
    current_transactions = utils.by(
        utils.multi_filter(
            predicates,
            Assistant.transactions.values()),
        key)
...
def restore_account_transactions(name=settings.account_name):
...
def restore_transactions(predicates, timestamp=settings.start_time, order='first', confirm=True):
