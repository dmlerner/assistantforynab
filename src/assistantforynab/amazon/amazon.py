...
def get_category_name(item):
    ...
    name = afy.settings.default_category
    return name
...
def has_blank_or_WIP_memo(t):
    return has_blank_memo(t) or afy.ynab.starts_with_id(t.memo)


def matches_account(t):
    return t.account_name.lower() == afy.settings.account_name.lower()  # TODO: remove explicit dependence on afy.settings
